		...
		function _reloadPrice() {
			...
			API.order.price(params)
				.then(function (priceInfo) {
					...
					$scope.all_discount = parseFloat(priceInfo.discount_price);
					for (var i = 0; i < priceInfo.promos.length; ++i){
						var promo = priceInfo.promos[i];
						$scope.all_discount += parseFloat(promo.price);
					}		
