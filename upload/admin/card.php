 ...
 * $Id: card.php 12844 2007-10-12 02:00:20Z hackfan $
*/
...
elseif ($_REQUEST['act'] == 'drop_card_img')
{
    ...
    if (!empty($img_name))
    {
        @unlink(ROOT_PATH . 'data/cardimg/' .$img_name);
...
elseif ($_REQUEST['act'] == 'edit_card_name')
{
    check_authz_json('card_manage');
    $card_id = empty($_REQUEST['id']) ? 0 : intval($_REQUEST['id']);
    $card_name = empty($_REQUEST['val']) ? '' : trim($_REQUEST['val']);
